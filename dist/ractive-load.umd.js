(function (global, factory) {
	...
	(global.Ractive = global.Ractive || {}, global.Ractive.load = factory());
...
var keywords$1 = /(case|default|delete|do|else|in|instanceof|new|return|throw|typeof|void)\s*$/;
var punctuators = /(^|\{|\(|\[\.|;|,|<|>|<=|>=|==|!=|===|!==|\+|-|\*\%|<<|>>|>>>|&|\||\^|!|~|&&|\|\||\?|:|=|\+=|-=|\*=|%=|<<=|>>=|>>>=|&=|\|=|\^=|\/=|\/)\s*$/;
var ambiguous = /(\}|\)|\+\+|--)\s*$/;
var beforeJsx = /^$|[=:;,\(\{\}\[|&+]\s*$/;
...
function find ( str ) {
	...
	function base ( char, i ) {
		if ( char === '/' ) {
			...
			if ( keywords$1.test( substr ) || punctuators.test( substr ) ) { regexEnabled = true; }
...
function parse$1 ( source, parseOptions, typeAttrs ) {
	if ( !Ractive$2 ) {
		throw new Error( 'rcu has not been initialised! You must call rcu.init(Ractive) before rcu.parse()' );
	}

	var parsed = Ractive$2.parse( source, Object.assign( {
		noStringify: true,
		interpolate: { script: false, style: false }
	}, parseOptions || {}, { includeLinePositions: true } ) );

	if ( parsed.v !== TEMPLATE_VERSION$1 ) {
		console.warn( ("Mismatched template version (expected " + TEMPLATE_VERSION$1 + ", got " + (parsed.v) + ")! Please ensure you are using the latest version of Ractive.js in your build process as well as in your app") ); // eslint-disable-line no-console
	}
...
function make ( source, config, callback, errback ) {
	...
	var definition = parse$1( source, parseOptions, typeAttrs );
	...
	function createComponent () {
		...
		if ( definition.script ) {
			...
			try {
				...
				var component = {};
				factory( component, config.require, Ractive$2 );
				...
				Component = Ractive$2.extend( options );
			} catch ( err ) {
				errback( err );
				return;
			}
...
//       var foo = new Foo(...);
//     });
function loadSingle$1 ( path, parentUrl, baseUrl, cache ) {
	var promise, url;
			...
			return new Promise( function ( fulfil, reject ) {
				make( template, {
					...
					loadImport: function ( name, path, parentUrl, callback ) {
						...
						// to load.baseUrl
						loadSingle$1( path, parentUrl, baseUrl, cache ).then( callback, reject );
					},
...
function ractiveRequire ( name ) {
	...
	dependency = load.modules.hasOwnProperty( name ) ? load.modules[ name ] :
	             global$1.hasOwnProperty( name ) ? global$1[ name ] : null;
...
function loadFromLinks ( baseUrl, cache ) {
	var promise = new Promise( function ( resolve, reject ) {
		...
		links = toArray$1( document.querySelectorAll( 'link[rel="ractive"]' ) );
		pending = links.length;

		links.forEach( function ( link ) {
			var name = getNameFromLink( link );

			loadSingle$1( link.getAttribute( 'href' ), '', baseUrl, cache ).then( function ( Component ) {
...
function loadMultiple ( map, baseUrl, cache ) {
	var promise = new Promise( function ( resolve, reject ) {
		...
		load = function ( name ) {
			...
			loadSingle$1( path, baseUrl, baseUrl, cache ).then( function ( Component ) {
...
function load ( url ) {
	...
	return loadSingle$1( url, baseUrl, baseUrl, cache );
}
